...
export default function PropertyPage() {
    ...
    return (
        <>
            ...
            {isComparablesModalOpen && property.comparables && typeof window !== 'undefined' && createPortal(
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                    <div
                        className="absolute inset-0 bg-black/60 backdrop-blur-sm"
                    ...
                    <div className="relative z-10 w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-gray-100">
                            <div>
                                <h2 className="text-2xl font-bold text-[#163331] flex items-center gap-3">
                                    <Ruler className="w-6 h-6 text-primary-500" />
                                    All Comparable Sales ({property.comparables.length})
                                </h2>
                                <p className="text-gray-600 mt-1">Properties sold in the area</p>
                            </div>
                            <button
                                onClick={() => setIsComparablesModalOpen(false)}
                                className="p-2 hover:bg-gray-100 rounded-full transition-colors"
                        ...
                        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                            <div className="space-y-4">
                                {property.comparables.map((sale, index) => (
                                    <div
                                        key={index}
                                        className="flex items-center gap-4 p-4 bg-gray-50 border border-gray-200 rounded-xl hover:border-primary-200 hover:shadow-md transition-all group"
                                    >
                                        <div className="w-20 h-20 rounded-lg flex-shrink-0 overflow-hidden bg-gray-200 relative">
                                            {sale.images && sale.images.length > 0 && sale.images[0]?.url ? (
                                                <img
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-4 mb-1">
                                                <div className="flex-1 min-w-0">
                                                    <div className="font-semibold text-[#163331] mb-0.5 line-clamp-1">
                                                        {sale.address}
                                                    </div>
                                                    <div className="text-xs text-gray-600 flex items-center gap-1">
                                                        <span>Sold {formatDate(sale.saleDate)}</span>
